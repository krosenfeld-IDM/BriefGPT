...
from streamlit_app_utils import (
    ...
    generate_answer,
    load_db_from_file_and_create_if_not_exists,
    validate_api_key,
    load_dir_chat_embeddings,
)
...
def summarize():
    ...
    if input_method == "Document":
        ...
        if files:
            ...
            st.write(
                "No documents found in documents folder. Add some documents first!"
            )
            return
    ...
    use_gpt_4 = st.checkbox(
        "Use GPT-4 for the final prompt (STRONGLY recommended, requires GPT-4 API access - progress bar will appear to get stuck as GPT-4 is slow)",
        value=True,
    )
...
def chat():
    ...
    if dir_or_doc == "Document":
        ...
        if st.button(
            "Load file (first time might take a second...) pressing this button will reset the chat history"
        ):
        ...
        if "text_input" not in st.session_state:
            st.session_state.text_input = ""
        load_or_create = st.checkbox(
            "Load from existing directory (already embedded)", value=False
        )
        if load_or_create:
            ...
            if load:
                embedding_file_path = os.path.join(
                    "directory_embeddings", select_embedding
                )
            ...
            directory = st.text_input(
                'Enter a directory to load from - just "documents" will load the default documents folder'
            )
            name = st.text_input("Enter a unique nickname for the directory")
            if st.button(
                "Load directory (first time might take a second...) pressing this button will reset the chat history"
            ):
...
def documents():
    st.title("Documents")
    st.markdown(
        "Documents are stored in the documents folder in the project directory."
    )
    directory = "documents"
    files = os.listdir(directory)
    files = [file for file in files if file.endswith(tuple(accepted_filetypes))]
    if files:
        files_df = pd.DataFrame(
            files, columns=["File Name"], index=range(1, len(files) + 1)
        )
...
def compare_results():
    st.title("Compare")
    st.write(
        "Compare retrieval results using hypothetical embeddings vs. normal embeddings. Support for comparing multiple models coming soon."
    )
    ...
    if st.button("Ask") and "db" in st.session_state and validate_api_key(model_name):
        ...
        answer_a, sources_a = generate_answer(
            st.session_state.db, model_name, hypothetical=True
        )
        answer_b, sources_b = generate_answer(
            st.session_state.db, model_name, hypothetical=False
        )
...
st.sidebar.markdown(
    "[More info on hypothetical embeddings here](https://arxiv.org/abs/2212.10496)",
